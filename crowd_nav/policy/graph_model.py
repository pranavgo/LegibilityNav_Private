...
class RGL(nn.Module):
    ...
    def forward(self, state):
        ...
        if not self.layerwise_graph:
            normalized_A = self.compute_similarity_matrix(X)
            self.A = normalized_A[0, :, :].clone().data.cpu().numpy()

        next_H = H = X
        for i in range(self.num_layer):
            if self.layerwise_graph:
                A = self.compute_similarity_matrix(H)
                next_H = relu(torch.matmul(torch.matmul(A.clone(), H.clone()), self.Ws[i].clone()), inplace=False)
            else:
                #next_H = relu(torch.matmul(torch.matmul(normalized_A, H), self.Ws[i]), inplace=False)
                next_H = self.my_relu(torch.matmul(torch.matmul(normalized_A, H), self.Ws[i]))
